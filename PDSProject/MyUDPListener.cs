    ...
    class MyUDPListener
    {
        ...
        public async Task Listener (CancellationToken token) {
            ...
            while (!token.IsCancellationRequested)
            {
                ...
                try {
                    ...
                    Console.WriteLine($"UDPListener SerializationException - UDP Packet is in the wrong format: {se.Message}");
                }
                catch (SocketException ex) {
                    Console.WriteLine($"UDPListenerError SocketException: {ex}");
                }
                catch (Exception ex) {
        ...
        async Task ServeClient(UdpReceiveResult result ) {
            try {
                ...
                Console.WriteLine($"UDPListener Exception - {se}");
            }
        }
        ...
        //                            MainWindow.main.StopTCPListener();// StartTCPListener();
        //                        }));
        //                    }
